...
3. Сколько байт в IP-заголовке? Сколько байт приходится на полезную нагрузку IP-дейтаграммы?
   ...
   - 56 - 20 = 36 байт полезной нагрузки
...
4. Отсортируйте отслеженные пакеты по их исходному IP-адресу; для этого щелкните по
   ...
   1. Какие поля IP-дейтаграммы всегда изменяются от одной дейтаграммы к
      ...
      - Identification (текущий идентификатор пакета), Time to Live (TTL) (он увеличивается), Header Checksum (пересчитывается из-за изменений заголовка).
   ...
   2. Какие поля не меняются? Какие поля должны оставаться неизменными? Какие
      ...
      - Не меняются Source IP, Destination IP, Version, Header Length, Total Length.
      - В рамках одного запуска неизменными должны оставаться Source IP, Destination IP, Version, Header Length и поле Protocol (ICMP).
      - Должны изменяться Time to Live (увеличивается на 1 с каждым новым запросом), Identification (инкрементируется) и поле Header Checksum (пересчитывается при изменении полей заголовка).
...
6. Остаются ли эти значения (Identification и TTL) неизменными во всех сообщениях
   ...
   - Нет значения постоянно меняются. При чем закономерности изменения идентификатора нет. TTL же скачет вокруг значений 60 и 250.
...
8. **Фрагментация**. Остановите захват пакетов в Wireshark. Измените размер пакета
   ...
   1. Было ли это сообщение фрагментировано между двумя или более IP-дейтаграммами? 
      ...
      - Да, об этом говорит наличие Fragment Offset не равный 0. Cтрока от Wireshark: [3 IPv4 Fragments (3480 bytes): #5(1480), #6(1480), #7(520)] говорит что было создано 3 фрагмента
![alt text](image-9.png)
Строка [3 IPv4 Fragments...] от Wireshark
   2. Какие поля IP-заголовка изменяются в разных фрагментах?
      - Total Length (последний короче)
      - Flags (у первых двух стоит More fragments=1, у последнего 0).
      - Fragment Offset (у 1-го = 0, у 2-го = 1480, у 3-го = 2960)
      - Header Checksum (т.к. разные поля то разная и контрольная сумма).
