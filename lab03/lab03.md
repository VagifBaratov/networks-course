...
#### Демонстрация работы
Запуск сервера и содержимое тестового файла:
![alt text](<images/Снимок экрана 2026-03-24 122928.png>)
Демонстрация запросов:
![alt text](<images/Снимок экрана 2026-03-24 122951.png>)
![alt text](<images/Снимок экрана 2026-03-24 123247.png>)
Повторный запрос файла, в случае если файл не менялся возвращаем 304:
![alt text](<images/Снимок экрана 2026-03-24 123104.png>)
Запрос несуществующего файла:
![alt text](<images/Снимок экрана 2026-03-24 171514.png>)
...
$t_{пакета} = \frac{56 * 8}{128 * 10^3} = \frac{448}{128000} = 0,0035с = 3,5 мс$     
Время передачи пакета в линию:   
$t_{передача} = \frac{448}{10^6} = 0,000448с = 0,448мс$
    
Время с момента создания бита до его декодирования:   
$T = t_{пакета} + t_{передача} + t_{распр} = 3,5 + 0,448 + 5 = 8,948 \approx 8,95мс $
...
$t_{передачи} = \frac{1}{100} = 0,01с = 10мс$
  
Средняя общая задержка:    
$d = t_{передачи} + t_{ожид} = 10 + 10 = 20мс = 0,02с$

Cредняя скорость поступления пакета в очередь:  
$a = \frac{N}{d} = \frac{11}{0,02} = 550 \ {пакетов/c}$
...
#### Решение
1. Время передачи одного пакета в первой линии: $t_S = \frac{L}{R_S}$. Пусть первый пакет начал передаваться в момент $t=0$. Значит последний бит первого пакета покинет сервер в момент $t_S$. Второй отправляется сразу после первого, значит последний бит второго покинет серевер в момент $2t_S$. Выходит по первой линии пакеты идут с интервалом $t_S$.    
$d_{\text{распространения}}$ не влияет на разницу во времени прибытия на клиент, т.к. оба пакета испытывают одинаковую задержку распространения на каждом участке.    
Вторая линия быстрее, поэтому пакеты не будут накапливаться перед ней, и второй пакет не будет ждать в очереди перед второй линией. А значит пакеты покидают маршрутизатор с тем же интервалом $t_S$.   
Т.о. интервал между окончанием приёма первого пакета клиентом и окончанием приёма второго пакета клиентом равен $t_S = \frac{L}{R_S}$
​
2. Теперь $R_C < R_S$. Первый пакет поступает на маршрутизатор из первой линии со скоростью $R_S$. Но т.к. выходная ск-ть $R_C < R_S$, пока первый пакет передаётся во вторую линию, второй пакет уже прибывает на маршрутизатор и вынужден ждать в буфере перед второй линией.   
Найдем $T$:    
Пусть передача первого пакета начинается в $t=0$.    
Окончание передачи первого пакета $t_S$.   
Начало передачи во вторую лиинию $t_S + d_{распространения}$.   
Конец передачи во торую линию $t_S + d_{распространения} + t_C$.   
Выходит второй пакет должен прибыть на маршрутизатор не раньше $t_S + d_{распространения} + t_C$.          
Пусть начало передачи второго пакета $T$. Окончание передачи $T + t_S$.   
Прибытие на маршрутизатор $T+ t_S +d_{распространения}$.    
Получили нер-во:  
$t_S + d_{распространения} + t_C \leq T+ t_S +d_{распространения}$   
$T \geq t_C = \frac{L}{R_C}$    
