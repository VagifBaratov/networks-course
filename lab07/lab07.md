...
1. Какой IP-адрес и номер порта TCP использует ваш компьютер (отправитель), передающий
   ...
   - IP-адрес: 192.168.1.47. Порт: 60032
   ![alt text](image.png)
...
3. Какой порядковый номер у SYN TCP-сегмента, который используется для установления
   ...
   - Порядковый номер: 2373285227
   - Можно видеть на скриншоте, что установлен флаг Syn
...
4. Какой порядковый номер у SYNACK-сегмента, отправленного сервером gaia.cs.umass.edu
   ...
   - Порядковый номер: 472173003
   - В поле подтверждения содержится: 2373285228
   ![alt text](image-4.png)
   - Сервер взял порядковый номер полученного SYN-пакета (2373285227) и прибавил к нему 1.
   - Установлены флаги Syn, Acknowledgment
...
6. Рассмотрите TCP-сегмент, содержащий команду POST протокола HTTP, как первый TCP-сегмент 
   ...
   - 2373285228![alt text](image-6.png)
     2373285877![alt text](image-7.png)
     2373287329![alt text](image-8.png)
     2373288781![alt text](image-9.png)
     2373290233![alt text](image-10.png)
     2373291685![alt text](image-11.png)
   - 11.92 - 12.04(для сегмента с Post)![alt text](image-12.png)![alt text](image-13.png)
     11.92 - 12.04(для оставшихся 5 сегментов приходит один ack-пакет) ![alt text](image-14.png)
   - RTT = 0.12 для всех сегментов
7. Чему равна пропускная способность (количество байтов, передаваемых в единицу
   ...
   - $\approx 68 920$ байт/с
   - Считаем время прошедшее с момента отправки первого syn-пакета до получения последнего ack-паета. После чего делим количество переданных байт на это время
