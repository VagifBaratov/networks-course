...
3. Рассмотрите три последних ICMP-пакета, полученных исходным хостом. Чем эти пакеты
   ...
   - Три последних пакета — это ICMP Echo Reply (Type 0, Code 0), а не Time Exceeded. И все они приходят от конечного хоста назначения, а не от маршрутизаторов.
   - Time Exceeded генерируюь только маршрутизаторы. Конечный хост же никогда не генерирует Time Exceeded, если пакет до него дошел, то он формирует корректный Echo Reply.
