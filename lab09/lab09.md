<img width="1920" height="1020" alt="image" src="https://github.com/user-attachments/assets/172ed58d-ec22-43c0-a340-54e95de9b958" /># Практика 9. Сетевой уровень
...
1. Каков IP-адрес вашего хоста? Каков IP-адрес хоста назначения?
   - Мой IP-адрес: 192.168.1.101
   - IP-адрес хоста назначения: 142.251.38.110
   ![alt text](images/image.png)
2. Почему ICMP-пакет    не обладает номерами исходного и конечного портов?
   - Потому что они ICMP - протокол сетевого уровня, где адресация осуществляется по IP-адресам. Понятие портов же принадлежит транспортному уровню. В ICMP-пакетах для идентификации есть поле «Identifier».
3. Рассмотрите один из ping-запросов, отправленных вашим хостом. Каковы ICMP-тип и кодовый
   номер этого пакета? Какие еще поля есть в этом ICMP-пакете? Сколько байт приходится на поля 
   контрольной суммы, порядкового номера и идентификатора?
   -  ICMP-тип: 8 (Echo request), Code: 0 
   - Checksum (контрольная сумма), Identifier (идентификатор), Sequence Number (порядковый номер), Data (полезные данные, в данном случае алфавит).
...
4. Рассмотрите соответствующий ping-пакет, полученный в ответ на предыдущий. 
   ...
   - ICMP тип: 0 (Echo reply), Кодовый номер: 0
   - Checksum (контрольная сумма), Identifier (идентификатор), Sequence Number (порядковый номер), Data (полезные данные, скопированные из запроса).
   -  Контрольная сумма - 2 байта, порядковый номер - 2 байта и идентификатор - 2 байта.
...
1. Рассмотрите ICMP-пакет с эхо-запросом на вашем скриншоте. Отличается ли он от ICMP-пакетов
   ...
   - Заголовки ICMP одинаковые. Различаются разве что TTL(у ping - 128), а так же нулевая Data(64 байта).
...
2. Рассмотрите на вашем скриншоте ICMP-пакет с сообщением об ошибке. В нем больше полей,
   ...
   - Зарезервированное поле (Unused, 4 байта), Оригинальный IP-заголовок пакета, который был отброшен, первые 64 байта данных отброшенного пакета.
...
3. Рассмотрите три последних ICMP-пакета, полученных исходным хостом. Чем эти пакеты
   ...
   - Три последних пакета — это ICMP Echo Reply (Type 0, Code 0), а не Time Exceeded. И все они приходят от конечного хоста назначения, а не от маршрутизаторов.<img width="1920" height="1020" alt="image" src="https://github.com/user-attachments/assets/f8814382-9c9c-4c89-a3bf-b49be0fece49" />

   - Time Exceeded генерируют только маршрутизаторы. Конечный хост же никогда не генерирует Time Exceeded, если пакет до него дошел, то он формирует корректный Echo Reply.
4. Есть ли такой канал, задержка в котором существенно превышает среднее значение? Можете
   ...
   - Можно заметить два скачка задержики: между маршрутизатором 8 и 10(3-14мс) и между 11 и 12(12-23мс).
   - 8 находится в РФ, наверное даже в Санкт-Петербурге, 10 - в Европе, а 12 видимо уже в США.
