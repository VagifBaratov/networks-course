...
Если считать что за один цикл роста от $\frac{W}{2}$ до $W$ происходит одна потеря. Тогда:   
$L \approx \frac{1}{N}$, где $N$ - число отправленых пакетов за цикл.  
$N = \frac{W}{2}\sum_0^{n-1}(1+\alpha)^i = \frac{W}{2} \cdot \frac{(1+\alpha)^n - 1}{(1+\alpha) -1} = \frac{W}{2} \cdot \frac{2 -1}{\alpha} = \frac{W}{2a}$   
Получаем что $L \approx \frac{W}{2a}$.  
