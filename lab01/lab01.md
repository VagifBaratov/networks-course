...
<img width="1920" height="1080" alt="Снимок экрана 2026-02-23 231858" src="https://github.com/user-attachments/assets/a2c19f55-0a0f-4bdb-9501-55103804f99f" />

#### Вопросы
1. Использует ли ваш браузер HTTP версии 1.0 или 1.1? Какая версия HTTP работает на
   сервере?
   - Мой браузер - HTTP 1.1
   - Сервер - HTTP 1.1
2. Какие языки (если есть) ваш браузер может принимать? В захваченном сеансе какую еще
   ...
   - Версию ядра системы, разрядность системы, версия браузера
3. Какой IP-адрес вашего компьютера? Какой адрес сервера gaia.cs.umass.edu?
   - 192.168.0.137 - мой пк
   - 128.119.245.12 - сервер
...
<img width="1920" height="1080" alt="Снимок экрана 2026-02-23 232341" src="https://github.com/user-attachments/assets/245cb653-54d2-4643-bb68-df1e8fcd0877" />

#### Вопросы
1. Проверьте содержимое первого HTTP-запроса GET. Видите ли вы строку «IF-MODIFIED-SINCE» в HTTP GET?
   - Нет, строки «IF-MODIFIED-SINCE» в HTTP GET нет
2. Проверьте содержимое ответа сервера. Вернул ли сервер содержимое файла явно? Как вы
   это можете увидеть?
   - Да, вернул содержимое
    <img width="1920" height="1080" alt="Снимок экрана 2026-02-23 232725" src="https://github.com/user-attachments/assets/620c325d-1832-47e6-a027-a98171566215" />
...
3. Теперь проверьте содержимое второго HTTP-запроса GET (из вашего браузера на сторону
   ...
   - Да, есть строка. If-Modified-Since: Tue, 28 Oct 2025 05:59:01 (последняя дата изменения нашей копии файла)
4. Какой код состояния HTTP и фраза возвращаются сервером в ответ на этот второй запрос
   ...
   - 304 Not Modified
   - Сервер не вернул явно содержимое файла
   <img width="1920" height="1080" alt="Снимок экрана 2026-02-23 233239" src="https://github.com/user-attachments/assets/6e4a4848-8b1b-42cd-8384-c2e30b0b045f" />
...
2. Какой номер пакета в трассировке содержит код состояния и фразу, связанные с ответом
   на HTTP-запрос GET?
   - 142
   <img width="1920" height="1080" alt="Снимок экрана 2026-02-24 000234" src="https://github.com/user-attachments/assets/01cbee91-2f5e-404a-975d-baf66fc5b769" />

3. Сколько сегментов TCP, содержащих данные, потребовалось для передачи одного HTTP ответа?
   - 4 сегмента TCP
   <img width="1920" height="1080" alt="Снимок экрана 2026-02-24 000735" src="https://github.com/user-attachments/assets/546edc95-827d-4d18-aa3c-2ee1f899827b" />

4. Есть ли в передаваемых данных какая-либо информация заголовка HTTP, связанная с
   сегментацией TCP?
   - Нет, http об этом ничего не знает
...
4. Остановите захват пакетов Wireshark и введите «http» в окне фильтра.
<img width="1920" height="1080" alt="Снимок экрана 2026-02-24 003202" src="https://github.com/user-attachments/assets/4279c73c-ef88-4cc0-8ced-4a4b5f34274d" />

#### Вопросы
1. Сколько HTTP GET запросов было отправлено вашим браузером? На какие интернет-адреса были отправлены эти GET-запросы?
   - 3 запроса
   - http://gaia.cs.umass.edu/wireshark-labs/HTTP-wireshark-file4.html; /pearson.png; /8E_cover_small.jpg
2. Можете ли вы сказать, загрузил ли ваш браузер два изображения последовательно или
   ...
   - Как можно видеть на скриншоте, запрос на загрузку второго сообщения был отправлен еще до подтверждения получения (или в моем случае редиректа на новый адрес). Следовательно загрузка была параллельной
...
<img width="1920" height="1080" alt="Снимок экрана 2026-02-24 003631" src="https://github.com/user-attachments/assets/7390cf05-e5e8-44b8-b985-bf6016660774" />
