...
1. Выполните nslookup, чтобы получить IP-адрес какого-либо веб-сервера в Азии
   - ![alt text](<images/Снимок экрана 2026-03-31 154400.png>)
2. Выполните nslookup, чтобы определить авторитетные DNS-серверы для какого-либо университета в Европе
   - Видим 6 авторитетных DNS-серверов: 
   ![alt text](<images/Снимок экрана 2026-03-31 154410.png>)
3. Используя nslookup, найдите веб-сервер, имеющий несколько IP-адресов. Сколько IP-адресов имеет веб-сервер вашего учебного заведения?
   - ![alt text](<images/Снимок экрана 2026-03-31 175555.png>)
   - Как видно всего один адрес:       
   ![alt text](<images/Снимок экрана 2026-03-31 154429.png>)
...
1. Найдите DNS-запрос и ответ на него. С использованием какого транспортного протокола
   они отправлены?
   - Ответ и запрос отправлены с использованием UDP 
   ![alt text](<images/Снимок экрана 2026-03-31 160731.png>)
   ![alt text](<images/Снимок экрана 2026-03-31 160748.png>)
2. Какой порт назначения у запроса DNS?
   - Порт 53
   ![alt text](<images/Снимок экрана 2026-03-31 160954.png>)
3. На какой IP-адрес отправлен DNS-запрос? Используйте ipconfig для определения IP-адреса
   ...
   - DNS-запрос отправлен на 192.168.1.1, что является адресом моего локального DNS-сервера.
   ![alt text](<images/Снимок экрана 2026-03-31 161202.png>)
4. Проанализируйте сообщение-запрос DNS. Запись какого типа запрашивается? Содержатся
   ...
   ![alt text](<images/Снимок экрана 2026-03-31 161457.png>)
5. Проанализируйте ответное сообщение DNS. Сколько в нем «ответов»? Что содержится в
   каждом?
   - 2 ответа.
   - В каждом содержится запись типа А с IP-адресом
   ![alt text](<images/Снимок экрана 2026-03-31 180450.png>)
6. Посмотрите на последующий TCP-пакет с флагом SYN, отправленный вашим компьютером.
   ...
   - Да, адрес соответсвует первой записи в DNS-ответе. 
   ![alt text](<images/Снимок экрана 2026-03-31 161832.png>)
7. Веб-страница содержит изображения. Выполняет ли хост новые запросы DNS перед
   загрузкой этих изображений?
   - Хост выполняет DNS-запрос для `static.ietf.org`
   ![alt text](<images/Снимок экрана 2026-03-31 162330.png>)
...
1. Каков порт назначения в запросе DNS? Какой порт источника в DNS-ответе?
   - В запросе - 53
   ![alt text](<images/Снимок экрана 2026-03-31 174106.png>)
   - В ответе - 53
   ![alt text](<images/Снимок экрана 2026-03-31 174115.png>)
2. На какой IP-адрес отправлен DNS-запрос? Совпадает ли он с адресом локального DNS-сервера, установленного по умолчанию?
   - Запрос отправлен на 192.168.1.1, что совпадает с адресом локального DNS-сервера, установленного по умолчанию.

3. Проанализируйте сообщение-запрос DNS. Запись какого типа запрашивается? Содержатся
   ли в запросе какие-нибудь «ответы»?
   - Запрашивается запись типа АААА (адрес IPv6).
   - Ответов не содержится.
   ![alt text](<images/Снимок экрана 2026-03-31 174150.png>)
4. Проанализируйте ответное сообщение DNS. Сколько в нем «ответов»? Что содержится в каждом?
   - Всего один ответ содержит одну CNAME-запись, которая сообщает, что www.spbu.ru является алиасом для spbu.ru.
   ![alt text](<images/Снимок экрана 2026-03-31 172747.png>)
...
1. На какой IP-адрес отправлен DNS-запрос? Совпадает ли он с адресом локального DNS-сервера, установленного по умолчанию? 
   ...
   - Запрос отправлен на 195.70.196.210
   ![alt text](<images/Снимок экрана 2026-03-31 165402.png>)
   - Как можно видеть этот адрес принадлежит `ns2.pu.ru`
   ![alt text](<images/Снимок экрана 2026-03-31 165921.png>)
2. Проанализируйте сообщение-запрос DNS. Запись какого типа запрашивается? Содержатся
   ли в запросе какие-нибудь «ответы»?
   - Запрашивается запись АААА.
   - Ответов не содержится.
   ![alt text](<images/Снимок экрана 2026-03-31 170136.png>)
3. Проанализируйте ответное сообщение DNS. Сколько в нем «ответов»? Что содержится в
   каждом?
   - Всего один ответ содержит одну CNAME-запись.
   ![alt text](<images/Снимок экрана 2026-03-31 170312.png>)
### Е. Сервисы whois (2 балла)
1. Что такое база данных whois?
   - Это база данных, которая содержит информацию об IP-адресах, доменных именах и их владельцах.
2. Используя различные сервисы whois в Интернете, получите имена любых двух DNS-серверов. 
   ...
   - Используя `https://who.is/whois`, получил имена еще двух DNS-серверов `spbu.ru`
  ![alt text](<images/Снимок экрана 2026-03-31 171009.png>)
3. Используйте команду nslookup на локальном хосте, чтобы послать запросы трем конкретным
   ...
   ![alt text](<images/Снимок экрана 2026-03-31 171556.png>)